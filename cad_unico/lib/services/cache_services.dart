...
class CacheService {
  ...
  static Future<void> cacheResponsaveis(List<ResponsavelModel> responsaveis) async {
    final prefs = await SharedPreferences.getInstance();
    final jsonList = responsaveis.map((r) => r.toJson()).toList();
    await prefs.setString(_responsaveisKey, jsonEncode(jsonList));
  }
  
  static Future<List<ResponsavelModel>?> getCachedResponsaveis() async {
    final prefs = await SharedPreferences.getInstance();
    final jsonString = prefs.getString(_responsaveisKey);
    if (jsonString != null) {
      final jsonList = jsonDecode(jsonString) as List;
      return jsonList.map((json) => ResponsavelModel.fromJson(json)).toList();
    }
    return null;
  }
}